...
Protokollversion: `rag-vs-web-1.1.0`  
...
verwendet. Kandidaten und Goldfelder wurden synthetisch quellengestützt
erzeugt. Der Study Owner hat die bestehenden 100 Datensätze vor dem ersten
Hauptaufruf exakt unverändert als `study_owner_pre_freeze_approval`
freigegeben. Es fand keine unabhängige klinische Prüfung dieses
Frage-/Goldsatzes statt.
...
kumulative Limit beträgt `STUDY_MAX_ESTIMATED_API_COST_USD=500.00` und umfasst
...
zulässigen Versuche an; sie ist keine vertragliche Anbieterpreisgarantie. Die
frühere 400-USD-Grenze ist vor dem Hauptlauf durch PD-002 abgelöst; Pilot- und
Vorbereitungskosten bleiben kumulativ erhalten und wurden nicht zurückgesetzt.
Die damaligen 400-USD-Artefakte bleiben versioniert archiviert.
...
## Study-Owner-Freeze und Abweichungen

Die 800 Hauptaufrufe beginnen ausschließlich, wenn
`outputs/study_phase2/questions/study_questions_frozen.jsonl` genau die 100
owner-freigegebenen Datensätze enthält und der Backend-Freeze belegt, dass
Fragetext, Gold-IDs, Coverage, Pflichtclaims und Quellenfelder gegenüber den
freigegebenen Dateien unverändert sind. Die leeren Reviewer-/Adjudikationsfelder
der Review-Arbeitsmappe werden weder interpretiert noch aufgefüllt.

PD-001 ersetzt vor dem ersten Hauptstudienaufruf das frühere Zwei-Reviewer-
Fragefreeze durch `study_owner_pre_freeze_approval`. Diese Änderung verringert
die Unabhängigkeit des Referenzstandards und wird als Limitation berichtet.
Sie ersetzt ausdrücklich nicht die späteren zwei unabhängigen, verblindeten
klinischen Antwortbewertungen, das Citation Audit oder die Adjudikation.
...
Die Fragen sind synthetisch, nur durch den Study Owner vorab freigegeben und
nicht unabhängig klinisch validiert; die 80/20-Mischung ist künstlich. Der lokale Corpus
