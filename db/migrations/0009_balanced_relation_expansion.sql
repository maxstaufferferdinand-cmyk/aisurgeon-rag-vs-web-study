-- Interleave relation categories so a finite linked-context budget cannot be
-- consumed entirely by a long adverse-reaction list before dosing/warning/
-- contraindication context is represented.
...
LANGUAGE sql STABLE PARALLEL SAFE AS $$
    WITH explicit_forward AS (
        ...
        UNION ALL SELECT * FROM table_parent_reverse
        UNION ALL SELECT * FROM medicinal_context
    ), deduplicated AS (
        ...
        SELECT *, row_number() OVER (
            PARTITION BY seed_id, typed_relation ORDER BY target_id
        ) AS relation_rank
        FROM deduplicated
    )
    SELECT seed_id, target_id, typed_relation, 'linked_context'::text
    FROM balanced
    ORDER BY array_position(p_seed_ids, seed_id), relation_rank,
             relation_priority, typed_relation, target_id
